...
> existing reference-implementation code onto the **v1** Agentfile model. It is
...
frontend** translates authored intent into `org.agentrc.*` OCI labels + layers.
...
executes `CMD`. Secrets are `LABEL org.agentrc.secret.*` references; the value
never enters the artifact.

## Module → v1 role mapping
...
| Current module / work | v1 role | Reads / emits |
|---|---|---|
| Agentfile parser (Go in `pkg/agentfile`; Python package) | **Frontend / compiler** — Agentfile → `org.agentrc.*` labels + layers | parses the four agentrc keywords + standard Dockerfile keywords; emits labels per spec §9 |
| `compile` / `build` tooling | **`agentrc build` / BuildKit frontend** — two front doors, identical OCI artifacts | emits OCI artifact with `org.agentrc.*` labels; `--policy-mode inline\|digest` |
| OCI manifest/config/layer push/pull scaffolding | **OCI labels & package** | layers carry `/mnt` resources; image config carries `org.agentrc.*` labels |
...
| Secret broker concepts (env / keyring / vault) | **Platform secret broker** | resolves `LABEL org.agentrc.secret.<name>=<scope>` references at run time; value never in the artifact |
...
| microVM / microsandbox experiments | **One substrate driver** (+ host-scoped secret model) | substrate-neutral; the platform picks it at run time |
| v2.5 proposal / branding assets | Background only | not normative |

## Authoring surface: four keywords, no legacy directives

The implementation historically recognized a broad directive family. In v1 that
...
**stale** and must be dropped from the parser. Their concerns map onto the v1
surface as follows:

| Legacy directive(s) | v1 replacement |
|---|---|
| `AGENT` / identity-from-`CMD` | `IDENTITY name=… version=… author=…` → `org.agentrc.identity.*` |
| `TOOL` / `TOOLSET` / `FUNCTION` | `COPY` / `ADD --remote … /mnt/tools/<name>` → `org.agentrc.tool.<name>` |
| `SKILL` | `COPY` / `ADD --remote … /mnt/skills/<name>` → `org.agentrc.skill.<name>` |
| `SERVER` / `MCP` | `COPY` / `ADD --remote … /mnt/mcp/<name>` → `org.agentrc.mcp.<name>` (+ `.origin`) |
| `URL` (egress) | `POLICY network dns:<host>:<port>` → `org.agentrc.network.dns.<host>` |
| `CRED` / `BROKER` | `LABEL org.agentrc.secret.<name>=host:<host>` (value never inline; broker injects) |
...
2. **`ADD --remote` delivery flags.** Implement `--cached` (default) /
   ...
   resource type.
3. **Label emission.** Emit `org.agentrc.*` labels exactly per spec §9 tables —
   `POLICY <key> <value>` → `org.agentrc.<key>=<value>`; embedded MCP / skills
...
6. **Fail-closed translation.** Any required boundary the platform cannot
   enforce, any required secret it cannot resolve, or any policy it cannot
   evaluate must **fail closed**, never silently drop.
7. **Secret broker.** Resolve `LABEL org.agentrc.secret.<name>=<scope>`
   references via the platform broker and inject at run time (host-scoped,
   microsandbox-style); the value must never enter the artifact, logs, or audit.
8. **OCI namespace.** Standardize on the `org.agentrc.*` label namespace and
   ...
   `io.agentrc.*` namespaces are **not** used in v1.
9. **Spec-version comment.** Use the parser-compatible syntax line
   `# syntax=agentrc.io/agentfile:v1` (replacing the old v0.1 syntax line);
   spec-version mentions read "v1 (Working Draft)", dated 2026-06-30.
...
2. Make the frontend and `arc build` emit identical `org.agentrc.*` labels +
   layers per spec §9.
...
5. Implement the platform secret broker over `org.agentrc.secret.*` references.
6. Migrate OCI annotations to `org.agentrc.*` + `vnd.agentrc.*`; drop legacy
   namespaces.
